...
def process_claim(claim_id, status):
    ...
    return redirect(url_for('Employee.process_claims'))
...
def view_claims():
    ...
    return render_template('claims.html', title='Claims', claims=claims, role=mysession.get("role"), current_page='process_claims')
